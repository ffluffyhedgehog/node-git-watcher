...
const removeDeployDir = (deployId) => new Promise((resolve, reject) => {
  shell.rm('-rf', path.join(DEPLOYS_PATH, deployId));
  return resolve();
});
          ...
          callbacks: {
            credentials: (url, userName) => {
              ...
              if (attempts < 3) {
                attempts++;
                return git
                  .Cred.userpassPlaintextNew(credentials.token,
                    'x-oauth-basic');
              }
              attempts = 0;
  ...
  repo.fetchAll({
    callbacks: {
      credentials: (url, userName) => {
        ...
        if (attempts < 3) {
          attempts++;
          return git.Cred.userpassPlaintextNew(config.credentials.token,
           'x-oauth-basic');
        }
        attempts = 0;
        return new Error('Wrong credentials');

...
const removeDeployFromConfig = require('./configFactory').removeDeployFromConfig;

const DEPLOYS_PATH = './deploys/';

// resolved with boolean - folder exists
//
const checkFolderAndCreate = (path) => {
...
const removeDeployDir = (deployId) => {
  return new Promise((resolve, reject) => {
...
const removeDeploy = (deployId, emitter, cb) => {
  return removeDeployDir(deployId)
...
const openRepoAndFetch = (config, emitter) =>　{
  git.Repository.open(path.resolve(__dirname, config.path))
    .then((repo) => {
      // Start watching
      const adaptedConfig = Object.assign(config, {
        path: DEPLOYS_PATH + config.deployId + '/',
      });
      resolve();

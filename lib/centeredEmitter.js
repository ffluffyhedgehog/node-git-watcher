...
/**
 Init new centeredEmitter instance
 @param {RemoteProxy} jstp - JSTP RemoteProxy instance to communicate
 with server
 @param {object} deploy - populated with deploy`s config
 @returns {EventEmitter} centeredEmitter instance
 */
module.exports = (jstp, deploy) => {
  ...
  emitter.lastDeployLog = '';
  emitter.isInitialized = false;

  emitter._deployLog = (log) => {
    emitter.lastDeployLog += log + '\n';
  };
  ...
  emitter.on('fetch.start', () => {
    emitter.lastDeployLog = 'Start of log.\n'; // Clearing last log
  ...
  emitter.on('build.err', (err) => {
    ...
    emitter._deployLog('Build error: ' + err.message);
  ...
  emitter.on('deploy.start', () => {
    ...
    emitter._deployLog('Starting application.');
  ...
  emitter.on('init', (cb) => {
    ...
    git.initAndDeploy(emitter.deploy, emitter)
      .then(() => cb(null))
      .catch((err) => cb(err));
  });
  emitter.on('fetch', (cb) => {
  ...
  emitter.on('remove', (cb) => {
    ...
    emitter.stopApp();
    git.removeDeploy(emitter.deploy.deployId, emitter, cb);
  });

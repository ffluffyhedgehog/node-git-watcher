...
const checkFileAndCreateSync = (path) => {
  try {
    fs.statSync(path);
  } catch (e) {
    if (e.errno === -2) {
...
const readDeploysFromFile = (config) => {
  ...
  checkFileAndCreateSync(DEPLOYS_FILE_PATH);
  try {
...
module.exports = {
  ...
  cookDeployConfig: (deployPath) => new Promise((resolve, reject) => {
    readJSONFromFileAsync(path.join(deployPath, DEFAULT_DEPLOY_CONFIG_NAME))
  ...
  removeDeployFromConfig: (deployId) => {
    return new Promise((resolve, reject) => {
      const deploys = readDeploysFromFile({})['deploys'];
